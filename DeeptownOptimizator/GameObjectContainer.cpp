...
GameObjectContainer::GameObjectContainer()
{
	...
	MyHelperUtils::read_objects_into(std::istringstream(sampleInputItems), items);
	...
	while (std::getline(strstream, line)) {
		...
		// Inputs, outputs
		auto inputs = get_inputs_outputs(line_split[0]);
		auto outputs = get_inputs_outputs(line_split[1]);
		// Processor for process
		const void* const ptr = MyHelperUtils::findInVectorByString(processors, line_split[2]);
		int time = std::stoi(line_split[3]);

		processes.emplace_back(inputs, outputs, time, (Processor*)ptr);
	}
